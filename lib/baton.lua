...
function Player:_updatePairs()
    if not self.config.joystick then
        for _, pair in pairs(self._pairs) do
            pair.rawX, pair.rawY = 0, 0
            pair.x, pair.y = 0, 0
    ...
    for _, pair in pairs(self._pairs) do
        -- get raw x and y
        local l = self._controls[pair.controls[1]] and self._controls[pair.controls[1]].rawValue or 0
        local r = self._controls[pair.controls[2]] and self._controls[pair.controls[2]].rawValue or 0
        local u = self._controls[pair.controls[3]] and self._controls[pair.controls[3]].rawValue or 0
        local d = self._controls[pair.controls[4]] and self._controls[pair.controls[4]].rawValue or 0
